...
@objc @implementation extension CylinderAnimator { 
    ...
    private func snake(_ page: SBIconListView, _ offset: CGFloat) {
        let percent = abs(offset / page.width)
        let maxColumns = page.iconColumnsForCurrentOrientation
        let maxRows = page.iconRowsForCurrentOrientation
        let maxIcons = maxColumns * maxRows
        
        // Use the first icon to determine margins/spacing
        var topLeftX: CGFloat = 0
        var topLeftY: CGFloat = 0
        var iconWidth: CGFloat = 60
        var iconHeight: CGFloat = 60

        page.enumerateIconViews { icon, i, stop in
            topLeftX = icon.frame.origin.x
            topLeftY = icon.frame.origin.y
            iconWidth = icon.frame.size.width
            iconHeight = icon.frame.size.height
            stop.pointee = true // Only need the first one
        }
        
        let maxTravelX = page.width - iconWidth - (topLeftX * 2)
        let maxTravelY = page.height - iconHeight - (topLeftY * 2)
        
        page.enumerateIconViews { icon, i, _ in
            var iconIndex = i 
            let iconRowNum = iconIndex / maxColumns
            
            // Snake path logic: reverse index on odd rows
            if iconRowNum % 2 == 1 {
                iconIndex = iconRowNum * maxColumns + (maxColumns - (iconIndex % maxColumns) - 1)
            }
            
            var iconPercent = percent
            if offset >= 0 {
                iconPercent += CGFloat(iconIndex) / CGFloat(maxIcons)
            } else {
                iconPercent += CGFloat(maxIcons - iconIndex - 1) / CGFloat(maxIcons)
            }
            
            let percentForRow = 1.0 / CGFloat(maxRows)
            var iconCurRowNum = Int(floor((iconPercent * CGFloat(maxIcons)) / CGFloat(maxColumns)))
            if iconCurRowNum > maxRows - 1 { iconCurRowNum = maxRows - 1 }
            
            var direction: CGFloat = 1.0
            if offset >= 0 {
                if iconCurRowNum % 2 == 1 { direction = -1.0 }
            } else {
                if (maxRows - iconCurRowNum - 1) % 2 == 0 { direction = -1.0 }
            }
            
            // Progress through row
            var percentThroughRow = (iconPercent - (percentForRow * CGFloat(iconCurRowNum))) * (1.0 / (percentForRow - (percentForRow / CGFloat(maxColumns))))
            percentThroughRow = max(0, min(1, percentThroughRow))
            
            // Exit animation logic for the very last icon in the sequence
            if iconPercent > CGFloat(maxIcons - 1) / CGFloat(maxIcons) {
                 percentThroughRow = 1.0 + (iconPercent - (CGFloat(maxIcons - 1) / CGFloat(maxIcons))) * (2.5 / (percentForRow - (percentForRow / CGFloat(maxColumns))))
            }
            
            if direction < 0 { percentThroughRow = 1.0 - percentThroughRow }
            
            // Progress through column
            var percentThroughColumn = CGFloat(iconCurRowNum) / CGFloat(maxRows - 1)
            if (percentForRow - (iconPercent - (percentForRow * CGFloat(iconCurRowNum))) < 1.0 / CGFloat(maxIcons)) {
                percentThroughColumn += (((iconPercent - (percentForRow * CGFloat(iconCurRowNum))) - ((1.0 / CGFloat(maxIcons)) * CGFloat(maxColumns - 1))) * CGFloat(maxIcons)) / CGFloat(maxRows - 1)
            }
            
            percentThroughColumn = max(0, min(1, percentThroughColumn))
            if offset < 0 { percentThroughColumn = 1.0 - percentThroughColumn }
            
            let endX = (percentThroughRow * maxTravelX) + topLeftX
            let endY = (percentThroughColumn * maxTravelY) + topLeftY
            
            let begX = icon.frame.origin.x
            let begY = icon.frame.origin.y
